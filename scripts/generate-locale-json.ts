 ...
 * and writes per-language JSON to public/locale/{lang}.json.
 *
 * These static files are served by Next.js and fetched/cached client-side,
 * eliminating the need to embed ~146 KB of translations in every page's
...
const LOCALE_OUT = path.resolve(__dirname, '../public/locale');
...
async function main() {
  ...
  console.log(`\nGenerated ${Object.keys(messages).length} locale files in public/locale/`);
}
